...
export default function ManajemenPesertaPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-imm-black">Manajemen Peserta</h1>
        <p className="text-sm text-imm-gray-dark">Kelola data peserta pendaftaran</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex items-center gap-2">
          <Filter size={16} className="text-imm-gray-dark" />
          <select
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value)}
            className="border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-imm-red focus:border-transparent outline-none"
          >
              ...
              {peserta.map((p, i) => (
                <tr key={p.id} className="hover:bg-imm-gray/50">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-xs font-medium px-2 py-1 rounded-full bg-blue-50 text-blue-700">{p.komisariat}</span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => handleDelete(p.id)} className="p-1.5 hover:bg-red-50 rounded-lg text-red-600" title="Hapus">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
              ...
              <>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {[
                        { label: "Email", value: selectedPeserta.email },
                        { label: "No. HP / WhatsApp", value: selectedPeserta.noHp },
                      ].map((item, i) => (
                        <div key={i} className="bg-gray-50 rounded-xl px-4 py-3">
                          <p className="text-[11px] text-imm-gray-dark uppercase tracking-wider mb-0.5">{item.label}</p>
                          <p className="text-sm font-medium text-imm-black">{item.value || "-"}</p>
