...
import { Plus, Pencil, Trash2, GripVertical, FileCheck } from "lucide-react";
...
export default function AdminPersyaratanPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/kegiatan?komisariat=${encodeURIComponent(adminKomisariat)}`)
      .then((r) => r.json())
      .then((data) => {
        setKegiatanList(data);
        if (data.length > 0 && selectedKegiatan === null) {
          setSelectedKegiatan(data[0].id);
        }
      })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-imm-black">Kelola Persyaratan</h1>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        ...
        {kegiatanList.length === 0 ? (
          <p className="text-sm text-imm-gray-dark">Belum ada kegiatan untuk komisariat {adminKomisariat}</p>
        ) : (
          <select
              ...
              {items.map((item) => (
                <div key={item.id} className="flex items-center justify-between px-6 py-4 hover:bg-imm-gray/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-1">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded-full ${item.wajib ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-600"}`}>
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => handleEdit(item)} className="p-2 hover:bg-blue-50 rounded-lg text-blue-600 transition-colors" aria-label="Edit">
                      <Pencil size={16} />
                    </button>
                    <button onClick={() => setDeleteTarget(item.id)} className="p-2 hover:bg-red-50 rounded-lg text-red-600 transition-colors" aria-label="Hapus">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
      ...
      <ConfirmDialog
        ...
        message="Yakin ingin menghapus persyaratan ini?"
        variant="danger"
